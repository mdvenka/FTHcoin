...
- [Development guidelines](#development-guidelines)
    - [General Faithcoin Core](#general-faithcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
([`valgrind.supp`](https://github.com/faithcoin-project/faithcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_faithcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_faithcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/faithcoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_faithcoin.coverage/index.html`.
```
...
- ThreadRPCServer : Remote procedure call handler, listens on port 9333 for connections and services them.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `faithcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `faithcoin-cli` and the GUI debug console use this table to determine how to
